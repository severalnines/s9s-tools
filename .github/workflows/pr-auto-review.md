...
name: PR Auto-Review
description: >
  Automatically approve any open, non-draft PR carrying the 'auto-review' label. If
  'service-account-teamcity' isn't already a requested reviewer, requests it first, then
  evaluates the diff for risk and approves. Posts a summary to #-dev-cc-general on Slack.
...
# Pinned to gpt-4 — gh-aw's copilot engine defaulted to claude-sonnet-4.6, which 400'd as
# unsupported against severalnines' Copilot Business plan. gpt-5 was then rejected by the
# installed Copilot CLI binary itself (v1.0.65) as "retired or unsupported" — its own error
# suggested gpt-4, which this CLI version's model list actually recognizes. Note: even with
# a working model, tool-calling isn't 100% reliable — a run can occasionally narrate an
# action without calling the tool. Not fully solved; worth watching over time.
engine:
  id: copilot
  model: gpt-4
...
draft, then assess risk and approve it — requesting yourself as reviewer first if you aren't
already one.
...
If `DRY_RUN` is `"true"`, print the intended approval instead and skip the tool call. Otherwise call
the `submit_pull_request_review` safe-output tool:
...
comment that @-mentions `MAINTAINER_GITHUB_LOGIN` so a human verifies before merge:
...
<if needs_human_review>⚠️ *Human review recommended:* <recommendation></if>
<if !needs_human_review>✅ No additional review needed</if>
```
...
```
🤖 PR Auto-Review — <repo>#<PR_NUMBER>   [DRY-RUN | APPROVED]
...
- A PR flagged `needs_human_review: true` is still approved (to unblock CI gates), but both the PR
  comment and the Slack summary explicitly call for human verification before merge.
- CRITICAL-risk PRs are approved with a strong "human must review" recommendation.
