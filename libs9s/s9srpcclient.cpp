        ...
        const int clusterId)
{
    ...
    // Add audit_events field if audit-log-event-data option is provided
...
S9sRpcClient::createCluster()
{
    ...
    hosts = options->nodes();
    if (options->clusterType() != "postgresql_logical"
            && hosts.empty())
    {
        PRINT_ERROR(
    ...
    } else if (options->clusterType() == "postgresql_logical")
    {
        success = createLogicalPostgreSql(
                hosts, osUserName, dbVersion);
 ...
 *   is an error message).
 *
 * This method will create a job that creates a logical PostgreSQL cluster of
 * streaming PostgreSQL clusters.
 */
bool
S9sRpcClient::createLogicalPostgreSql(
        const S9sVariantList &hosts,
        const S9sString      &osUserName,
        const S9sString      &psqlVersion)
{
    ...
    if (hosts.size() < 1u && !options->hasClustersOption())
    {
        PRINT_ERROR("Either node list or clusters list should be specified when creating PostgreSQL logical cluster.");
        return false;
    }
    ...
    // The job_data describing the cluster.
    //
    jobData["cluster_type"]     = "postgresql_logical";
    jobData["type"]             = "postgresql";
    jobData["vendor"]           = options->vendor();
    if (options->hasEnterpriseToken())
        jobData["enterprise_token"] = options->enterpriseToken();
    ...
    if (options->hasClustersOption())
        jobData["clusters"] = options->clusters();
    ...
    // The job instance describing how the job will be executed.
    //
    job["title"]                = "Creating PostgreSQL Logical Cluster";
    job["job_spec"]             = jobSpec;
 ...
 */
bool
S9sRpcClient::createElasticsearch(
        const S9sVariantList &hosts,
        const S9sString      &osUserName,
        const S9sString      &version)
{
    ...
    if (hosts.size() < 1u)
    {
        PRINT_ERROR("Missing node list while creating Elasticsearch cluster.");
        return false;
    }
    ...
    // The job_data describing the cluster.
    //
    jobData["cluster_type"]     = "elastic";
    jobData["type"]             = "elastic";
    jobData["nodes"]            = nodesField(hosts);
    jobData["version"]          = version;
    jobData["db_user"]          = options->dbAdminUserName();
    jobData["db_password"]      = options->dbAdminPassword();
    ...
    // The job instance describing how the job will be executed.
    //
    job["title"]                = "Creating Elasticsearch Cluster";
    job["job_spec"]             = jobSpec;
...
S9sRpcClient::rebootHost()
{
    ...
    // The jobspec describing the command.
    jobSpec["command"]    = "reboot";
    jobSpec["job_data"]   = jobData;

    // The job instance describing how the job will be executed.
    job["title"]          = "Rebooting Node";
    job["job_spec"]       = jobSpec;
...
S9sRpcClient::checkHosts()
{
    ...
    S9sString const accessCmd    = options->accessCheckCmd();
    ...
    if (!accessCmd.empty())
        request["access_check_cmd"] = accessCmd;
        ...
        bool argumentsAreContainers) const
{
    ...
    if (!options->accessCheckCmd().empty())
        jobData["access_check_cmd"] = options->accessCheckCmd();
...
bool
S9sRpcClient::registerElasticsearchCluster(
        const S9sVariantList &hosts,
        const S9sString      &osUserName)
{
    S9sOptions     *options = S9sOptions::instance();
    ...
    else if (hosts.size() > 1u)
    {
        ...
        PRINT_ERROR("Elasticsearch clusters requires only one cluster's node to discover topology");
        return false;
    }
    ...
    {
        PRINT_ERROR("Database user is required for Elasticsearch cluster");
        return false;
    }
    jobData["db_user"]          = options->dbAdminUserName();
    if(options->dbAdminPassword().empty())
    {
        PRINT_ERROR("Database password is required for Elasticsearch cluster");
        return false;
    }
    jobData["db_password"]      = options->dbAdminPassword();
    ...
    // The job instance describing how the job will be executed.
    //
    job["title"]                = "Register Elasticsearch Cluster";
    job["job_spec"]             = jobSpec;
